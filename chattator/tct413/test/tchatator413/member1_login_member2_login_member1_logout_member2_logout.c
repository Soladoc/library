...
#define STOP()                       \
    ...
    if (!test_case_o_file_fmt(&test, obj_output, OUT_JSON(NAME, "1"), &gs_tokens[0], &gs_tokens[1])) STOP();

    json_object_put(obj_input);
    obj_input = load_input_jsonf(IN_JSONF(NAME, "2"), gs_tokens[0], gs_tokens[1]);
    json_object_put(obj_output);
