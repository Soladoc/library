...
struct test test_tchattator413_zero(cfg_t *cfg, db_t *db, server_t *server) {
    ...
    test_case(&t, streq(json_output, zero_O), "json output == expected");
...
#define admin_whois_1_I "[{\"do\":\"whois\",\"with\":{\"api_key\":\"" ADMIN_API_KEY "\",\"user\":1}}]"
...
struct test test_tchattator413_admin_whois_1(cfg_t *cfg, db_t *db, server_t *server) {
    ...
    char const *json_output = json_object_to_json_string_ext(obj_output, JSON_C_TO_STRING_PLAIN);
    test_case(&t, streq(json_output, admin_whois_1_O), "json output == expected");
...
#define invalid_whois_1_I "[{\"do\":\"whois\",\"with\":{\"api_key\":\"" ADMIN_API_KEY "\",\"user\":1}}]"
...
struct test test_tchattator413_invalid_whois_1(cfg_t *cfg, db_t *db, server_t *server) {
    ...
    json_object *obj_output = tchattator413_interpret(obj_input, cfg, db, server, invalid_whois_1_on_action, invalid_whois_1_on_response, &t);

    char const *json_output = json_object_to_json_string_ext(obj_output, JSON_C_TO_STRING_PLAIN);
    test_case(&t, streq(json_output, invalid_whois_1_O), "json output == expected");
