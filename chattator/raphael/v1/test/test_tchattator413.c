...
#define admin_whois_neg1_O OUTPUT_500
...
#define invalid_login_I "[{\"do\":\"login\",\"with\":{\"api_key\":\"" INVALID_API_KEY_REPR "\",\"password\":""}}]"
#define invalid_login_O "[{\"status\":401,\"has_next_page\":false,\"body\":{}}]"
...
struct test test_tchattator413_invalid_login(cfg_t *cfg, db_t *db, server_t *server) {
    ...
    json_object *obj_input = json_tokener_parse(invalid_login_I);
    test_case_i(test, obj_input, invalid_login_I);
    ...
    test_case_o(test, obj_output, invalid_login_O);
