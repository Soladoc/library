...
static void admin_whois_imax_on_action(action_t const *action, void *t) {
    begin_on_action(t);
...
static void admin_whois_imax_on_response(response_t const *response, void *t) {
    begin_on_response(t);
...
struct test test_tchattator413_admin_whois_imax(cfg_t *cfg, db_t *db, server_t *server) {
    ...
    json_object *obj_output = tchattator413_interpret(obj_input, cfg, db, server, admin_whois_imax_on_action, admin_whois_imax_on_response, &test);
