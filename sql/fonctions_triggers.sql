...
create or replace function offres_insert()
    returns trigger
    as $$
declare
    id_temp integer;
...
begin
    insert into pact._identite default
        values
        returning
            id_identite into id_temp;
    insert into pact._compte(email, mdp_hash, nom, prenom, telephone, id_identite, existe)
        values (new.email, new.mdp_hash, new.nom, new.prenom, new.telephone, id_temp, true);
    insert into pact._membre(pseudo, email)
        values (new.pseudo, new.email);
    return new;
end;
$$
language 'plpgsql';
...
begin
    ...
    insert into pact._compte(email, mdp_hash, nom, prenom, telephone, id_identite, existe)
        values (new.email, new.mdp_hash, new.nom, new.prenom, new.telephone, id_temp, true);
    insert into pact._professionnel(denomination, email)
        values (new.denomination, new.email)
    returning
        id_professionnel into id_temp;
    insert into pact._prive(id_prive, siren)
        values (id_temp, new.siren);
    return new;
end;
$$
language 'plpgsql';
...
create or replace function nb_offres_en_ligne(id_pro_cherche integer)
    returns table(
        id_offre integer,
        titre varchar(255),
        resume varchar(1023),
...
declare
    ...
    boucle integer;
begin
    create view _offres as
    select
        *
    from
        _offre
    where
        id_professionnel = id_pro_cherche;
...
create or replace function nb_offres_hors_ligne(id_pro_cherche integer)
    returns table(
        id_offre integer,
        titre varchar(255),
        resume varchar(1023),
...
create or replace function categorie(id_offre_cherche integer)
    returns varchar (
        20
)
    as $$
...
begin
    ...
    if (
        ...
        from
            _restaurant
        where
            id_offre = id_offre_cherche) > 0 then
...
create or replace function note(id_offre_cherche integer)
    returns float
    as $$
declare
    note float;
